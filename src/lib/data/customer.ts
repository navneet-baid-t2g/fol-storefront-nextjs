...
export async function generateResetPasswordToken(_currentState: unknown, formData: FormData) {
  const email = formData.get("email") as string

  try {
    // First check if customer exists
    const checkResponse = await fetch(`${process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL}/store/custom?email=${encodeURIComponent(email)}`, {
      method: "GET",
      headers: {
    ...
    // Customer exists, send reset email
